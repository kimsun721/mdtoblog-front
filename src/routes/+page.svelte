...
<main class="min-h-screen" style="background: var(--bg);">
	<div class="mx-auto max-w-3xl px-5 py-12 sm:px-8">
		...
		<header class="mb-10">
			...
			<p class="mt-1 text-sm" style="color: var(--text-muted);">개발자 블로그</p>
		</header>
		...
		{:else}
			...
			<div class="space-y-3">
				{#each posts as post (post.id)}
					<article
						class="group cursor-pointer rounded-xl border p-5 transition-all duration-150 hover:shadow-md"
					...
					>
						<h2
							class="mb-2 text-[15px] font-semibold leading-snug transition-colors duration-150 group-hover:opacity-60"
							style="color: var(--text);"
						>
							{post.title}
						</h2>

						<p class="mb-4 line-clamp-2 text-[13px] leading-relaxed" style="color: var(--text-secondary);">
						...
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-2 text-[12px]" style="color: var(--text-muted);">
								<button
									type="button"
									class="cursor-pointer flex items-center gap-1.5 transition hover:opacity-100"
